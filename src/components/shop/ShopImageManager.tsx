...
export function ShopImageManager({ products }: { products: ProductItem[] }) {
  ...
  async function upload(productId: string, file: File) {
    ...
    try {
      ...
      setStatus(response.ok ? "Image updated. Refresh the shop to see it." : result.error ?? `Upload failed (${response.status})`);
    } catch {
      setStatus("Upload could not connect. Refresh the page and try again.");
    } finally {
      setUploading(null);
    }
  }
  ...
  return (
    <details className="mb-8 rounded-2xl border border-cream-border bg-white p-5 shadow-sm">
      ...
      <p className="mt-2 text-xs leading-5 text-brand-muted">Private admin tool. Upload a JPG, PNG, or WebP product photo up to 5MB.</p>
